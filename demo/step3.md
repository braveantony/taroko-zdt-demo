...
step2 的問題:SSE 長連線本來就不會自己結束,關機時等也等不完,撞 shutdown 上限被剪斷。

**這步加的**(跟 step2 比,唯一差別):`HYDRA_SSE_DRAIN=on`。app 關機時主動對每條 SSE 送出
`bye` 事件,然後由 server 端關閉連線。手上沒有掛著的長連線,`Shutdown` 幾秒內就能完成、exit 0。

**背後的 code**:關機流程在 `Shutdown` 之前先排空 SSE,三個檔案串起來、一路走到底:
...
// handler.go:SSE handler 看到 bye channel 關閉 → 送 bye 事件、收線 return
case <-bye:
...
同一段關機流程,`SSE_DRAIN` 只決定「要不要走 `Hub().Drain()` 那一步」——差別全在這裡:
...
| client 看到 | stream 中途被剪斷、**沒有 bye** | 先收到 **`bye`**、連線照預期收掉 |
...
> 指令用精簡寫法;請先照 [step0 的前置作業](step0.md) 設好 `kind-zdt` context 與 `zdt-tour` namespace,
...
預期:SSE stream 會先收到一個 `bye` 事件**才**關閉——是 server 主動收線、你能預期它什麼時候斷,不是硬斷。

- 用 `curl` 看:收到 `bye` 後連線正常結束、指令跟著結束(curl 不會自動重連)。
- 用瀏覽器看(想看導覽頁的話,另開終端機 `kubectl port-forward svc/hydra 8080:80`,開 <http://localhost:8080/tour>):`EventSource` 一偵測到連線斷掉就會自動重連,燈號由「導覽員換班中」轉回「導覽中」。
...
> - 連線一開始 server 送過一行 `retry: 1000`(SSE 規格定義的),那是**建議 client 的重連間隔**(1 秒);動作還是
>   client 做,不送就用瀏覽器預設(多數瀏覽器約 3 秒)。
> - 所以 `drain=on` 真正改善的是讓 **pod 早點乾淨放手(exit 0)**,順帶給前端一個 `bye` 更新 UI;client 端的
>   重連兩種情況本來都會發生,不是靠 drain。

重點不是「TCP 連線永不中斷」,而是中斷變得可預期。

## 觀察二:進度還是掉了(接下來要解的問題)

排空解決了連線,但進度仍在 pod 的記憶體裡。做法:開一個帶 cookie 的 session、記住走到第幾站,
滾動更新後用**同一個 cookie** 重連,看進度還在不在。cookie 寫在 client pod 的 `/tmp/jar`;
client 不在這次 rollout 的範圍,所以兩次 exec 讀到的是同一個檔。

(其實記憶體狀態本來就跟「建立它的那顆 pod」綁死:三個副本又沒做 session affinity,就算不 rollout、
單純重連也可能被分到別顆 pod 而讀不到進度。rollout 只是保證舊 pod 一定不在了,比較好觀察。)
...
# 2) 觸發滾動更新,並等它換完(確保待會重連的是新 pod,而不是還沒被換掉的舊 pod)
kubectl rollout restart deploy/hydra
kubectl rollout status deploy/hydra
